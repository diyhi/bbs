 ...
 *
 */
@Component("sensitiveWordFilterComponent")
public class SensitiveWordFilterComponent {
	private static final Logger logger = LogManager.getLogger(SensitiveWordFilterComponent.class);
	...
	public String filterSensitiveWord(String content,String replaceChar){
		...
		if(findedWords != null && findedWords.size() >0){
			...
			//	content = replace(content, str, replaceChar);
				content = Strings.CI.replace(content, str, replaceChar);//忽略大小写替换； String.replaceAll不能直接替换特殊字符，例如：$/
			}
		}
		return content;
	}
    ...
    public boolean isUpdate(){
    	String path = "data"+File.separator+"filterWord"+File.separator+"word.txt";
