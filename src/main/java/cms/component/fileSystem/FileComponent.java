package cms.component.fileSystem;
...
public class FileComponent {
    ...
    /**
     * 获取文件服务器随机地址 如http://s3-1.diyhi.com/
    ...
    public String fileServerAddress(HttpServletRequest request){
        if(request != null){
            return WebUtil.getUrl(request);//本地文件存储系统
        }
        return "";
    }
    /**
    ...
    public List<String> fileServerAllAddress(HttpServletRequest request){
        if(request != null){
            ...
            newEndpoint.add(WebUtil.getUrl(request)); //本地文件存储系统
            return newEndpoint;
        }
        return null;
    }
    ...
    public String processRichTextFilePath(String html,String item){
        ...
        if(html != null && !html.trim().isEmpty()){
            return textFilterComponent.processFilePath(html, item, this.fileServerAddress(request),true);
        }
    ...
    public Boolean createFolder(String path){
        //正在使用的文件存储系统
        int useFileSystem =  this.getFileSystem();
        if(useFileSystem == 0){
            //本地文件存储系统
    ...
    public void writeFile(String path,String newFileName,byte[] content){
        //本地文件存储系统
        localFileComponent.writeFile(path, newFileName, content);
    ...
    public void copyFile(String resFilePath, String distFolder) throws IOException {
        //本地文件存储系统
        localFileComponent.copyFile(resFilePath,distFolder);
    ...
    public void copyDirectory(String resDirectory, String distFolder) throws IOException {
        //本地文件存储系统
        localFileComponent.copyDirectory(resDirectory, distFolder);
    }
    ...
    public Boolean deleteFile(String path){
        //本地文件存储系统
        return localFileComponent.deleteFile(path);
    }
    ...
    public Boolean removeDirectory(String path){
        //本地文件存储系统
        return localFileComponent.removeDirectory(path);
    }
    ...
    public void failedStateFile(String path){
        ...
        if(useFileSystem == 0){
            //本地文件存储系统
            localFileComponent.failedStateFile(path);
        }
    }
    ...
    public void addLock(String path,String lockFileName)throws IOException{
        ...
        //本地文件存储系统
        localFileComponent.addLock(path, lockFileName);
    }
    ...
    public void deleteLock(String path,String fileName){
        //本地文件存储系统
        localFileComponent.deleteLock(path, fileName);
    }
    ...
    public void addThumbnail(List<Thumbnail> thumbnailList, List<ImageInfo> imageInfoList){
        ...
        int useFileSystem =  this.getFileSystem();
        if(useFileSystem == 0){
            thumbnailComponent.addThumbnail(thumbnailList, imageInfoList);
        }
    ...
    public void deleteThumbnail(List<Thumbnail> thumbnailList,List<ImageInfo> imageInfoList){
        ...
        if(useFileSystem == 0){
            //本地文件存储系统
            thumbnailComponent.deleteThumbnail(thumbnailList, imageInfoList);
        }
    }
    ...
    public void createImage(InputStream sourceInputStream,String outputPath,String extension,int scaleWidth,int scaleHeight) {
        //本地文件存储系统
        thumbnailComponent.createImage(sourceInputStream, PathUtil.defaultExternalDirectory()+ File.separator+outputPath, extension, scaleWidth, scaleHeight);
    }
    /**
     * 生成缩略图
     * @param sourceInputStream 源图片文件流
    ...
    public void createImage(InputStream sourceInputStream,String outputPath,String extension,int x,int y,int width,int height,int scaleWidth,int scaleHeight) {
        //本地文件存储系统
        thumbnailComponent.createImage(sourceInputStream, PathUtil.defaultExternalDirectory()+File.separator+outputPath, extension, x, y, width, height, scaleWidth, scaleHeight);
    }
    ...
    public void deleteInvalidFile(){
        ...
        if(systemSetting != null){
            if(systemSetting.getTemporaryFileValidPeriod() != null && systemSetting.getTemporaryFileValidPeriod() >0){
                ...
                localFileComponent.lockRemoveFile("topic",maxDeleteTime);//删除话题文件
                localFileComponent.lockRemoveFile("comment",maxDeleteTime);//评论文件
                localFileComponent.lockRemoveFile("question",maxDeleteTime);//删除问题文件
                localFileComponent.lockRemoveFile("answer",maxDeleteTime);//答案文件

                localFileComponent.lockRemoveFile("help",maxDeleteTime);//删除帮助文件
                localFileComponent.lockRemoveFile("helpType",maxDeleteTime);//删除帮助分类文件
                localFileComponent.lockRemoveFile("links",maxDeleteTime);//删除友情链接文件
                localFileComponent.lockRemoveFile("membershipCard",maxDeleteTime);//会员卡

                localFileComponent.lockRemoveFile("report",maxDeleteTime);//删除举报图片文件

                localFileComponent.lockRemoveFile("topicTag",maxDeleteTime);//删除话题标签图片文件
                localFileComponent.lockRemoveFile("questionTag",maxDeleteTime);//删除问答标签图片文件
                localFileComponent.lockRemoveFile("frontendApi",maxDeleteTime);//删除前台Api文件
    ...
    /**
     * 是否允许运行互相同步文件
     * @return interfaceProduct 接口产品 10.SeaweedFS 20.MinIO 30.阿里云OSS
     * @return 0:允许  3:同步运行过程中，不能执行本次任务 4:第三方文件存储系统没启用
     */
    public int allowSyncFile(int interfaceProduct){
        return 0;
    }
    ...
    public String createSignLink(String link,String fileName,String secret,Long expires){
        ...
        if(WebUtil.getPath() != null && !WebUtil.getPath().trim().isEmpty()){
            //删除虚拟路径
            newSecureLink = Strings.CI.removeStart(newSecureLink, WebUtil.getPath()+"/");//移除开始部分的相同的字符,不区分大小写
        }
