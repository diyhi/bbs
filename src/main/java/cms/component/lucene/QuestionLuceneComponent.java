...
import cms.config.SplitAnalyzer;
 ...
 *
 */
@Component("questionLuceneComponent")
public class QuestionLuceneComponent {
	private static final Logger logger = LogManager.getLogger(QuestionLuceneComponent.class);
	
	@Resource TextFilterComponent textFilterComponent;
    @Resource SearcherManager questionSearcherManager;
    @Resource IndexWriter questionIndexWriter;
		...
		if(questionList != null && questionList.size() >0){
			...
			try {
				...
				for(Question question : questionList){
                    ...
                    doc.add(new SortedDocValuesField("title", new BytesRef(question.getTitle())));
					...
					questionContent = textFilterComponent.filterText(questionContent.trim());

                    // 内容  Lucene 会自动使用配置的 Analyzer 进行分词
                    doc.add(new TextField("content", questionContent, Field.Store.YES));
				//	doc.add(new SortedDocValuesField("content", new BytesRef(productInfo.getContent())));//排序
					...
					if(!_questionTagIdGroup.trim().isEmpty()){
						//问题标签Id组
					...
					//发表时间
					doc.add(new LongPoint("postTime",question.getPostTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli()));
					doc.add(new NumericDocValuesField("postTime",question.getPostTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli()));//要排序，必须添加一个同名的NumericDocValuesField
                    ...
                    questionIndexWriter.addDocument(doc);
				}

                questionIndexWriter.commit();//提交数据
	...
	public QueryResult<Question> findIndexByCondition(int firstIndex, int maxResult,
        ...
        try {
            searcher = questionSearcherManager.acquire();
        ...
        //	e.printStackTrace();
            ...
            //关闭资源
            try {
                questionSearcherManager.release(searcher);
            } catch (IOException e) {
                if (logger.isErrorEnabled()) {
	...
	public void deleteUserNameIndex(List<String> userNameList){
		...
		try { 
            ...
            questionIndexWriter.deleteDocuments(query.build());//删除指定ID的Document
            questionIndexWriter.forceMergeDeletes();
            questionIndexWriter.commit();//提交
		} catch (IOException e) {
	...
	public void deleteIndex(List<Long> questionIdList){
		try {
			...
			if(questionIdList != null && questionIdList.size() >0){
				...
				if(q != null){
                    questionIndexWriter.deleteDocuments(q);//删除指定ID的Document
	...
	public void deleteAllIndex(){
		try {
            questionIndexWriter.deleteAll();//标识删除全部
            questionIndexWriter.forceMergeDeletes();  //强制删除    通过此方法可以强制从物理文件上删除文档，强制优化，del文件就没了，回收站清空
            questionIndexWriter.commit();//提交数据
    ...
    public void refreshIndex(){
        try {
            questionSearcherManager.maybeRefresh();
        } catch (IOException e) {
            // TODO Auto-generated catch block
            //	e.printStackTrace();
            if (logger.isErrorEnabled()) {
