package cms.component.payment.impl.mobile;
...
public class AlipayConfig_Mobile {
	...
	//请求网关沙箱地址
	//public String URL = "https://openapi-sandbox.dl.alipaydev.com/gateway.do"; //https://openapi.alipaydev.com/gateway.do
	// 编码
	...
	private synchronized void update(Integer interfaceProduct,Integer version){
		...
		for(OnlinePaymentInterface onlinePaymentInterface : onlinePaymentInterfaceList){
			//接口产品 
			if(onlinePaymentInterface.getInterfaceProduct().equals(interfaceProduct)){
				if(onlinePaymentInterface.getDynamicParameter() != null && !onlinePaymentInterface.getDynamicParameter().trim().isEmpty()){
					Alipay alipay = jsonComponent.toObject(onlinePaymentInterface.getDynamicParameter(), Alipay.class);
