 ...
 *
 */
@Component("userLoginLogComponent")
public class UserLoginLogComponent {
	private static final Logger logger = LogManager.getLogger(UserLoginLogComponent.class);
    ...
    public Object createUserLoginLogObject(UserLoginLog userLoginLog){
		...
		if(tableNumber == 0){//默认对象为UserLoginLog
			...
			Class<?> c;
			try {
				c = Class.forName("cms.model.user.UserLoginLog_"+tableNumber);
                //获取无参数构造函数
