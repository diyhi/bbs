...
public class CustomUserDetailsService implements UserDetailsService {
	...
	@Resource StaffRepository staffRepository;//通过接口引用代理返回的对象
	...
	public UserDetails loadUserByUsername(String username)
		...
		SysUsers user = staffCacheManager.query_cache_findByUserAccount(username);
        ...
        //如果是管理员则包含所有权限
        if(user.isIssys()){
            auths = staffCacheManager.query_allAuthorities();
        }else{
            //得到用户的权限
            auths = staffCacheManager.query_userAuthoritiesByName(user.getUserAccount());
        }
