...
public class Upgrade4_9to5_0 {
    ...
    public static void run(String upgradeId){
    	...
    	for(int i =0; i< 100; i++){
    		...
    		if(upgradeSystem.getRunningStatus()>=100 && upgradeSystem.getRunningStatus()<200){
    			...
    			updateSQL_sendSmsLog_platformUserId(upgradeService);
    			upgradeService.addLog(upgradeId, JsonUtils.toJSONString(new UpgradeLog(new Date(),"表SendSmsLog将userName列的值复制到platformUserId列成功",1))+",");
	...
	/**
   	 * 修改短信发送错误日志表字段,将userName列的值复制到platformUserId列
   	 * @param upgradeService
   	 */
    private static void updateSQL_sendSmsLog_platformUserId(UpgradeService upgradeService){
    	String sql = "UPDATE sendsmslog SET platformUserId=userName";
