...
@Repository
@Transactional
public class CommentRepositoryImpl extends DaoSupport<Comment> implements CommentRepository {
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	public List<Comment> findCommentByPage(String userName, LocalDateTime postTime, int firstIndex, int maxResult){
	...
	public void saveComment(Comment comment){
		this.save(comment);
		topicRepository.addCommentTotal(comment.getTopicId(), 1L);
	}
	 ...
	 * @return
	 */
	public Integer updateComment(Long commentId,String content,boolean isMarkdown,String markdownContent,Integer status,LocalDateTime lastUpdateTime,String lowerQuoteIdGroup,String userName){
		Query query = em.createQuery("update Comment o set o.content=?1,o.isMarkdown=?2,o.markdownContent=?3,o.userName=?4,o.status=?5,o.lastUpdateTime=?6 where o.id=?7")
		...
		.setParameter(6, lastUpdateTime)
		.setParameter(7, commentId);
	 ...
	 * @return
	 */
	public Integer updateComment(Long commentId,String content,String markdownContent,Integer status,LocalDateTime lastUpdateTime,String userName){
		Query query = em.createQuery("update Comment o set o.content=?1,o.markdownContent=?2,o.userName=?3,o.status=?4,o.lastUpdateTime=?5 where o.id=?6")
		.setParameter(1, content)
		.setParameter(2, markdownContent)
		.setParameter(3, userName)
		.setParameter(4, status)
		.setParameter(5, lastUpdateTime)
		.setParameter(6, commentId);
	 ...
	 * @return
	 */
	public int updateCommentPendingReview(Long commentId){
		Query query = em.createQuery("update Comment o set o.status=?1 where o.id=?2 and o.status=?3")
				.setParameter(1, 10)
				.setParameter(2, commentId)
				.setParameter(3, 20);
		return query.executeUpdate();
	...
	*/
	public Integer deleteComment(Long topicId,Long commentId){
		...
		topicRepository.subtractCommentTotal(topicId,1L);
		return i;
	} 
	...
	*/
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	public List<Reply> findReplyByPage(String userName,LocalDateTime postTime,int firstIndex, int maxResult){
	...
	*/
	 ...
	 * @return
	 */
	public int updateReplyPendingReview(Long replyId){
		Query query = em.createQuery("update Reply o set o.status=?1 where o.id=?2 and o.status=?3")
				.setParameter(1, 10)
				.setParameter(2, replyId)
				.setParameter(3, 20);
		return query.executeUpdate();
