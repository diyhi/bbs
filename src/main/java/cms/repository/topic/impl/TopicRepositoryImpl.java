...
@Repository
@Transactional
public class TopicRepositoryImpl extends DaoSupport<Topic> implements TopicRepository {
	...
	public List<Topic> findTopicByTopicIdList(List<Long> topicIdList){
		...
		if(objectList != null && objectList.size() >0){
			for(int i = 0; i<objectList.size(); i++){
                ...
                LocalDateTime postTime = (LocalDateTime)obj[4];
	...
	public List<Topic> findTopicByPage(String userName,LocalDateTime postTime,int firstIndex, int maxResult){
		...
		if(objectList != null && objectList.size() >0){
			for(int i = 0; i<objectList.size(); i++){
				...
				Long tagId = (Long)obj[2];
				String content = (String)obj[3];
                LocalDateTime _postTime = (LocalDateTime)obj[4];
	...
	public List<Topic> findHotTopic(int maxResult){
		...
		String sql = "select o.id,o.title,o.commentTotal,o.viewTotal" +
				" from Topic o where o.status=?1 ORDER BY o.weight desc";
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	@Cacheable(value="topicRepositoryImpl_cache_findHotTopic",key="'findHotTopic_default'")
	...
	public Integer updateTopic(Topic topic,VoteTheme addVoteTheme,VoteTheme editVoteTheme,String deleteVoteThemeId,List<VoteOption> addVoteOptionList,List<VoteOption> editVoteOptionList,List<String> deleteVoteOptionIdList){
		Query query = em.createQuery("update Topic o set o.title=?1, o.content=?2,o.isMarkdown=?3,o.markdownContent=?4,o.summary=?5,o.tagId=?6,o.tagIdGroup=?7,o.allow=?8,o.image=?9,o.status=?10,o.sort=?11,o.essence=?12,o.voteThemeId=?13 where o.id=?14")
		.setParameter(1, topic.getTitle())
		.setParameter(2, topic.getContent())
		.setParameter(3, topic.getIsMarkdown())
		.setParameter(4, topic.getMarkdownContent())
		.setParameter(5, topic.getSummary())
		.setParameter(6, topic.getTagId())
		.setParameter(7, topic.getTagIdGroup())
		.setParameter(8, topic.isAllow())
		.setParameter(9, topic.getImage())
		.setParameter(10, topic.getStatus())
		.setParameter(11, topic.getSort())
		.setParameter(12, topic.getEssence())
		.setParameter(13, topic.getVoteThemeId())
		.setParameter(14, topic.getId());
	...
	public Integer updateTopic2(Topic topic,VoteTheme addVoteTheme,VoteTheme editVoteTheme,String deleteVoteThemeId,List<VoteOption> addVoteOptionList,List<VoteOption> editVoteOptionList,List<String> deleteVoteOptionIdList){
		Query query = em.createQuery("update Topic o set o.title=?1, o.content=?2,o.markdownContent=?3, o.summary=?4,o.image=?5,o.status=?6,o.lastUpdateTime=?7,o.voteThemeId=?8 where o.id=?9 and o.status < ?10")
		.setParameter(1, topic.getTitle())
		.setParameter(2, topic.getContent())
		.setParameter(3, topic.getMarkdownContent())
		.setParameter(4, topic.getSummary())
		.setParameter(5, topic.getImage())
		.setParameter(6, topic.getStatus())
		.setParameter(7, topic.getLastUpdateTime())
		.setParameter(8, topic.getVoteThemeId())
	...
	public Integer markDelete(Long topicId){
		int i = 0;
		Query query = em.createQuery("update Topic o set o.status=o.status+100 where o.id=?1 and o.status<?2")
		.setParameter(1, topicId)
		.setParameter(2, 100);
	...
	public Integer deleteTopic(Long topicId,GiveRedEnvelope giveRedEnvelope,String userName,BigDecimal amount,Object paymentLogObject){
		...
		//删除提醒
		remindRepository.deleteRemindByTopicId(topicId);
		
		//删除收藏
		favoriteRepository.deleteFavoriteByTopicId(topicId);
		
		if(paymentLogObject != null){
			//返还红包
			redEnvelopeRepository.refundRedEnvelope(giveRedEnvelope,userName,amount,paymentLogObject);
		}
	...
	public int updateTopicPendingReview(Long topicId){
		Query query = em.createQuery("update Topic o set o.status=?1 where o.id=?2 and o.status=?3")
				.setParameter(1, 10)
				.setParameter(2, topicId)
				.setParameter(3, 20);
		return query.executeUpdate();
	...
	public void saveTopicUnhide(Object topicUnhide, Integer hideTagType, Long point, String consumption_userName, Object consumption_pointLogObject, String income_userName, Object income_pointLogObject,
		...
		if(hideTagType.equals(HideTagType.POINT.getName()) && point != null && point >0L){//积分购买
			//扣减用户积分
			int i = userRepository.subtractUserPoint(consumption_userName,point, consumption_pointLogObject);
			if(i >0){
				this.save(topicUnhide);
				if(income_pointLogObject != null){
					//增加用户积分
					userRepository.addUserPoint(income_userName,point,income_pointLogObject);
				}
			}
		}
		if(hideTagType.equals(HideTagType.AMOUNT.getName()) && consumption_amount != null && consumption_amount.compareTo(new BigDecimal("0")) >0){//余额购买
			//扣减用户预存款
			int i = userRepository.subtractUserDeposit(consumption_userName, consumption_amount, consumption_paymentLogObject);
			if(i >0){
				this.save(topicUnhide);
				if(income_paymentLogObject != null){
					userRepository.addUserDeposit(income_userName, userShare_amount, income_paymentLogObject);
				}
	...
	public TopicUnhide findTopicUnhideById(String topicUnhideId){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.topic.TopicUnhide_"+tableNumber);
                //获取无参数构造函数
