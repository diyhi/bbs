...
@Repository
@Transactional
public class MembershipCardRepositoryImpl extends DaoSupport<MembershipCard> implements MembershipCardRepository {
		...
		if(i >0){
			...
			if(update_specificationList != null && update_specificationList.size() >0){
				for(Specification specification : update_specificationList){
					...
					if(!inventory_where_sql.isEmpty()){
	...
	public void saveMembershipCardOrder(MembershipCardOrder membershipCardOrder, UserRoleGroup add_userRoleGroup, UserRoleGroup update_userRoleGroup,
		...
		if(membershipCardOrder.getPaymentAmount().compareTo(new BigDecimal("0")) >0){
			int i = userRepository.subtractUserDeposit(membershipCardOrder.getUserName(),membershipCardOrder.getPaymentAmount(),paymentLog);
			if(i==0){
				throw new BusinessException("扣除用户金额错误");
			}
		}

		//扣除用户积分
		if(membershipCardOrder.getPaymentPoint() >0L){
			int i = userRepository.subtractUserPoint(membershipCardOrder.getUserName(),membershipCardOrder.getPaymentPoint(),pointLog);
			if(i==0){
				throw new BusinessException("扣除用户积分错误");
			}
		}
		...
		if(j ==0){
			throw new BusinessException("扣减库存错误");
