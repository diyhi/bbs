package cms.repository.favorite.impl;
...
import cms.component.favorite.FavoritesConfig;
import cms.component.favorite.QuestionFavoriteConfig;
import cms.component.favorite.TopicFavoriteConfig;
...
@Repository
@Transactional
public class FavoriteRepositoryImpl extends DaoSupport<Favorites> implements FavoriteRepository {
	private static final Logger logger = LogManager.getLogger(FavoriteRepositoryImpl.class);
	...
	public Favorites findById(String favoriteId){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.favorite.Favorites_"+tableNumber);
                //获取无参数构造函数
	...
	public TopicFavorite findTopicFavoriteById(String topicFavoriteId){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.favorite.TopicFavorite_"+tableNumber);
                //获取无参数构造函数
                Constructor<?> constructor = c.getDeclaredConstructor();
	...
	public QuestionFavorite findQuestionFavoriteById(String questionFavoriteId){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.favorite.QuestionFavorite_"+tableNumber);
                //获取无参数构造函数
                Constructor<?> constructor = c.getDeclaredConstructor();
