package cms.repository.staff.impl;
...
@Repository
@Transactional
public class StaffRepositoryImpl extends DaoSupport implements StaffRepository {
	 private static final Logger logger = LogManager.getLogger(StaffRepositoryImpl.class);
	
	@Resource ACLRepository aclRepository;//通过接口引用代理返回的对象
	...
	public List<GrantedAuthority> loadAllAuthorities(){
		try {
			List<GrantedAuthority> auths = new ArrayList<GrantedAuthority>();
			List<SysPermission> sysPermissionList = aclRepository.findPermissionList();
	...
	private Integer deleteStaffLoginLog(String staffId){
		...
		int j = 0;
		int staffLoginLog_tableNumber = staffLoginLogShardingConfig.getTableQuantity();
	...
	public QueryResult<StaffLoginLog> findStaffLoginLogPage(String staffId, int firstIndex, int maxResult){
		...
		//表编号
		int tableNumber = staffLoginLogShardingConfig.staffIdRemainder(staffId);
		if(tableNumber == 0){//默认对象为HistoryOrder
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.staff.StaffLoginLog_"+tableNumber);
                //获取无参数构造函数
