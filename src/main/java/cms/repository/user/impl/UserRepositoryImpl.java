...
import cms.component.user.UserDynamicConfig;
...
@Repository
@Transactional
public class UserRepositoryImpl extends DaoSupport<User> implements UserRepository {
	private static final Logger logger = LogManager.getLogger(UserRepositoryImpl.class);
	
	@Resource PointLogConfig pointLogConfig;
	@Resource UserGradeRepository userGradeRepository;

	
	@Resource UserLoginLogConfig userLoginLogConfig;

	@Resource PrivateMessageRepository privateMessageRepository;
	@Resource SystemNotifyRepository systemNotifyRepository;
	...
	public List<User> findUserInfoByConditionPage(String jpql,List<Object> params,int firstIndex, int maxResult){
		...
		if(objectList != null && objectList.size() >0){
			for(int o = 0; o<objectList.size(); o++){
				...
				user.setRegistrationDate(ObjectConversion.conversion(object[4], ObjectConversion.LOCALDATETIME));
	...
	public QueryResult<User> findUserByCondition(String param, List<Object> paramValue, int firstIndex, int maxResult, boolean sort){
		...
		if(objectList != null && objectList.size() >0){
			for(int o = 0; o<objectList.size(); o++){
				...
				user.setRegistrationDate(ObjectConversion.conversion(object[8], ObjectConversion.LOCALDATETIME));
	...
	public QueryResult<User> findUserByCustomCondition(String param,List<Object> paramValue,String customParam,int firstIndex, int maxResult,boolean sort){ 
		...
		//mysql强制使用索引:force index(索引名或者主键PRI)
		String sql ="select t2.id, t2.userName,t2.account,t2.nickname, t2.mobile,t2.cancelAccountTime, t2.avatarName, t2.email, t2.registrationDate, t2.point, t2.state,t2.type,t2.platformUserId from(select u.userId from  user o, userinputvalue u where "+customParam+param+" group by u.userId having count(u.userId)>=? "+orderBy+" limit ?,?)t1,user t2 where t1.userId=t2.id";
	...
	public Integer updateUser(User user, List<UserInputValue> add_userInputValue,List<Long> delete_userInputValueIdList, List<UserRoleGroup> userRoleGroupList){
		...
		userRoleRepository.updateUserRoleGroup(user.getUserName(), userRoleGroupList);
	...
	public Integer delete(List<Long> idList,List<String> userNameList){
		...
		//删除私信
		privateMessageRepository.deleteUserPrivateMessage(idList);
		//删除用户所有订阅系统通知
		systemNotifyRepository.deleteUserSubscriptionSystemNotify(idList);
		
		
		//删除用户提醒
		remindRepository.deleteRemindByUserId(idList);
		
		//删除收藏夹
		favoriteRepository.deleteFavoriteByUserName(userNameList);
		
		//删除点赞
		likeRepository.deleteLikeByUserName(userNameList);
		

		//删除关注
		followRepository.deleteFollowByUserName(userNameList);
		
		//根据话题/问题的用户名称删除收藏
		favoriteRepository.deleteFavoriteByPostUserName(userNameList);

		//删除用户动态
		this.deleteUserDynamic(userNameList);
		
		//删除用户角色组
		userRoleRepository.deleteUserRoleGroup(userNameList);
		
		//删除问题
		questionRepository.deleteQuestion(userNameList);
		
		//删除答案
		answerRepository.deleteAnswer(userNameList);
		
		//删除回复
		answerRepository.deleteAnswerReply(userNameList);
		//删除问题标签关联
		questionRepository.deleteQuestionTagAssociationByUserId(userNameList);
		
		//删除发红包
		redEnvelopeRepository.deleteGiveRedEnvelope(idList);
		
		//删除收红包
		redEnvelopeRepository.deleteReceiveRedEnvelope(idList);
		
		//删除会员卡赠送项
		membershipCardGiftTaskRepository.deleteMembershipCardGiftItemByUserName(userNameList);
	...
	public Integer onlineRecharge(String paymentRunningNumber,String userName,BigDecimal deposit,Object paymentLog){
		int j = 0;
		//删除校验支付校验日志
		int i = paymentRepository.deletePaymentVerificationLogById(paymentRunningNumber);
		if(i >0){
	...
	public Integer addUserDeposit(String userName,BigDecimal deposit,Object paymentLog){
		...
		if(i >0){
			//保存支付日志
			if(paymentLog != null){
				paymentRepository.savePaymentLog(paymentLog);
			}
		}
	...
	public PointLog findPointLogById(String pointLogId){
		...
		if(tableNumber == 0){//默认对象为HistoryOrder
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.user.PointLog_"+tableNumber);
                //获取无参数构造函数
	...
	public UserLoginLog findUserLoginLogById(String userLoginLogId){
		...
		if(tableNumber == 0){//默认对象为HistoryOrder
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.user.UserLoginLog_"+tableNumber);
                //获取无参数构造函数
                Constructor<?> constructor = c.getDeclaredConstructor();
	 ...
	 * @param endTime 结束时间
	 */
	public void deleteUserLoginLog(LocalDateTime endTime){
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	@Cacheable(value="userRepositoryImpl_disableUserName_cache",key="'findAllDisableUserName_default'")
	 ...
	 */
	@CacheEvict(value="userRepositoryImpl_disableUserName_cache",allEntries=true)
...
/**----------------------------------- 用户动态 -------------------------------------**/
	...
	public QueryResult<UserDynamic> findUserDynamicPage(Long userId,String userName,int firstIndex, int maxResult){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.user.UserDynamic_"+tableNumber);
                //获取无参数构造函数
                Constructor<?> constructor = c.getDeclaredConstructor();
