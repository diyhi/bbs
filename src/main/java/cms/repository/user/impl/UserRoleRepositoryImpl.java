...
@Repository
@Transactional
public class UserRoleRepositoryImpl extends DaoSupport<UserRole> implements
        UserRoleRepository {
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	@Cacheable(value="userRoleRepositoryImpl_cache",key="'findAllRole_default'")
	 ...
	 * @param userRole 用户角色
	 */
	@CacheEvict(value="userRoleRepositoryImpl_cache",allEntries=true)
	...
	public Integer updateUserRole(UserRole userRole){
		Query query = em.createQuery("update UserRole o set o.name=?1,o.sort=?2, o.remark=?3,o.userResourceFormat=?4,o.multiLanguageExtension=?5 where o.id=?6")
			...
			.setParameter(5, userRole.getMultiLanguageExtension())
			.setParameter(6, userRole.getId());
	 ...
	 */
	public Integer updateUserRoleGroup(String userRoleId, String userName, LocalDateTime validPeriodEnd){
