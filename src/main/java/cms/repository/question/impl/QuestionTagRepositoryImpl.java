...
@Repository
@Transactional
public class QuestionTagRepositoryImpl extends DaoSupport<QuestionTag> implements QuestionTagRepository {
	...
	*/
	...
	public void saveQuestionTag(QuestionTag questionTag){
		...
		if(!questionTag.getParentId().equals(0L)){//如果不是根节点
			...
			if(j >0){
				//将父节点下的问题转到本节点
				questionRepository.updateTagId(questionTag.getParentId(), questionTag.getId());
			}
		}
	...
	public Integer updateQuestionTag(QuestionTag questionTag){
		Query query = em.createQuery("update QuestionTag o set o.name=?1, o.sort=?2,o.image=?3,o.multiLanguageExtension=?4 where o.id=?5")
		.setParameter(1, questionTag.getName())
		.setParameter(2, questionTag.getSort())
		.setParameter(3, questionTag.getImage())
		.setParameter(4, questionTag.getMultiLanguageExtension())
		.setParameter(5, questionTag.getId());
		int i = query.executeUpdate();
		return i;
	}
	...
	public Integer deleteQuestionTag(QuestionTag questionTag){
		...
		if(i >0){
			//删除问题标签关联
			questionRepository.deleteQuestionTagAssociation(questionTag.getId());
	...
	/**
	 * 递归删除所有子节点
	 * @param questionTagIdList 标签Id集合
	 */
		...
		for(Long tagId : questionTagIdList){
			...
			if(tagList != null && tagList.size() >0){
				...
				for(QuestionTag t : tagList){
					...
					if(s >0){
						//删除问题标签关联
						questionRepository.deleteQuestionTagAssociation(t.getId());
					}
