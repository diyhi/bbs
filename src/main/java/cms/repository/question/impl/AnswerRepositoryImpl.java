...
@Repository
@Transactional
public class AnswerRepositoryImpl extends DaoSupport<Answer> implements AnswerRepository {

	@Resource QuestionRepository questionRepository;
	@Resource UserRepository userRepository;
	@Resource PlatformShareRepository platformShareRepository;
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	public List<Answer> findAnswerByPage(String userName, LocalDateTime postTime, int firstIndex, int maxResult){
	...
	public void saveAnswer(Answer answer){
		this.save(answer);
		questionRepository.addAnswerTotal(answer.getQuestionId(), 1L);
	}
	 ...
	 * @param answerId 答案Id
	 * @param content 内容
	 * @param isMarkdown 是否为markdown内容
	 * @param markdownContent markdown内容
	 * @param status 状态
	 * @param userName 用户名称
	 * @return
	 */
	public Integer updateAnswer(Long answerId,String content,boolean isMarkdown,String markdownContent,Integer status,String userName){
		Query query = em.createQuery("update Answer o set o.content=?1,o.isMarkdown=?2,o.markdownContent=?3,o.userName=?4,o.status=?5 where o.id=?6")
		.setParameter(1, content)
		.setParameter(2, isMarkdown)
		.setParameter(3, markdownContent)
		.setParameter(4, userName)
		.setParameter(5, status)
		.setParameter(6, answerId);
	 ...
	 * @return
	 */
	public Integer updateAnswer(Long answerId,String content,String markdownContent,Integer status,LocalDateTime lastUpdateTime,String userName){
		Query query = em.createQuery("update Answer o set o.content=?1,o.markdownContent=?2,o.userName=?3,o.status=?4,o.lastUpdateTime=?5 where o.id=?6")
		.setParameter(1, content)
		.setParameter(2, markdownContent)
		.setParameter(3, userName)
		.setParameter(4, status)
		.setParameter(5, lastUpdateTime)
		.setParameter(6, answerId);
		return query.executeUpdate();
	}
	/**
	 ...
	 * @return
	 */
	public int updateAnswerPendingReview(Long answerId){
		Query query = em.createQuery("update Answer o set o.status=?1 where o.id=?2 and o.status=?3")
				.setParameter(1, 10)
				.setParameter(2, answerId)
				.setParameter(3, 20);
		return query.executeUpdate();
	...
	public int updateAdoptionAnswer(Long questionId, Long answerId, boolean changeAdoption, String cancelAdoptionUserName, Object cancelAdoptionPointLogObject, BigDecimal cancelAdoptionUserNameShareAmount, Object cancelAdoptionPaymentLogObject,
		...
		if(i >0){
			//修改问题的采纳答案Id
			questionRepository.updateAdoptionAnswerId(questionId, answerId);
			if(changeAdoption){//如果是更改采纳
				...
				//删除旧的平台分成
				platformShareRepository.deleteQuestionRewardPlatformShare(questionId, cancelAdoptionUserName);
				//保存新的平台分成
	...
	private void rewardPayment(String userName,Long point,Object pointLogObject,BigDecimal amount,Object paymentLogObject){
		if(point != null && point >0L){//积分
			//增加用户积分
			userRepository.addUserPoint(userName,point,pointLogObject);

		}
		if(amount != null && amount.compareTo(new BigDecimal("0")) >0){//余额
			//增加用户预存款
			userRepository.addUserDeposit(userName, amount, paymentLogObject);
		}
	...
	private void rewardReturn(String cancelAdoptionUserName,Object cancelAdoptionPointLogObject,BigDecimal cancelAdoptionUserNameShareAmount,Object cancelAdoptionPaymentLogObject,Long point){
		if(cancelAdoptionPointLogObject !=null && point != null && point >0L){//积分
			//扣减用户积分
			int i = userRepository.subtractUserPoint(cancelAdoptionUserName,point, cancelAdoptionPointLogObject);
			if(i == 0){
		...
		if(cancelAdoptionPaymentLogObject != null && cancelAdoptionUserNameShareAmount != null && cancelAdoptionUserNameShareAmount.compareTo(new BigDecimal("0")) >0){//余额
			//扣减用户预存款
			int i = userRepository.subtractUserDeposit(cancelAdoptionUserName, cancelAdoptionUserNameShareAmount, cancelAdoptionPaymentLogObject);
			if(i ==0){
				throw new BusinessException("扣减预存款失败");
			}
		}
	...
	public int updateCancelAdoptionAnswer(Long questionId,String cancelAdoptionUserName,Object cancelAdoptionPointLogObject,BigDecimal cancelAdoptionUserNameShareAmount,Object cancelAdoptionPaymentLogObject,Long point){
		...
		query.executeUpdate();
		//修改问题的采纳答案Id
		int i = questionRepository.updateAdoptionAnswerId(questionId, 0L);
	...
	public Integer deleteAnswer(Long questionId,Long answerId,String cancelAdoptionUserName,Object cancelAdoptionPointLogObject,BigDecimal cancelAdoptionUserNameShareAmount,Object cancelAdoptionPaymentLogObject,Long point){
		...
		if(i >0){
			...
			if(cancelAdoptionUserName != null){
				//修改问题的采纳答案Id
				questionRepository.updateAdoptionAnswerId(questionId, 0L);
		...
		questionRepository.subtractAnswerTotal(questionId,1L);
		return i;
	} 
	...
	*/
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	public List<AnswerReply> findReplyByPage(String userName,LocalDateTime postTime,int firstIndex, int maxResult){
	...
	*/
	 ...
	 * @return
	*/
	public Integer updateReply(Long replyId,String content,String userName,Integer status,LocalDateTime lastUpdateTime){
	 ...
	 * @return
	 */
	public int updateAnswerReplyPendingReview(Long answerReplyId){
		Query query = em.createQuery("update AnswerReply o set o.status=?1 where o.id=?2 and o.status=?3")
				.setParameter(1, 10)
				.setParameter(2, answerReplyId)
				.setParameter(3, 20);
		return query.executeUpdate();
