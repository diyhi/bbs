package cms.repository.redEnvelope.impl;
...
@Repository
@Transactional
public class RedEnvelopeRepositoryImpl extends DaoSupport<GiveRedEnvelope> implements RedEnvelopeRepository {
	private static final Logger logger = LogManager.getLogger(RedEnvelopeRepositoryImpl.class);
	...
	public void saveGiveRedEnvelope(GiveRedEnvelope giveRedEnvelope, String userName, BigDecimal amount, PaymentLog paymentLog){
		...
		if(amount != null && amount.compareTo(new BigDecimal("0")) >0){//余额
			...
			//扣减用户预存款
			int i = userRepository.subtractUserDeposit(userName, amount, paymentLogObject);
			if(i ==0){
	...
	public void refundRedEnvelope(GiveRedEnvelope giveRedEnvelope,String userName,BigDecimal amount,Object paymentLogObject){
		if(amount != null && amount.compareTo(new BigDecimal("0")) >0){//余额
			...
			if(i >0){
				...
				//扣减用户预存款
				int j = userRepository.addUserDeposit(userName, amount, paymentLogObject);
				if(j ==0){
					throw new BusinessException("增加预存款失败");
				}
			}else{
				throw new BusinessException("返还红包失败");
			}
		}
	}
    ...
    */
    public Integer unwrapRedEnvelope(String receiveRedEnvelopeId,BigDecimal amount,Integer version,String userName,Object paymentLogObject){
    	...
    	if(i >0){
			//增加用户预存款
			j = userRepository.addUserDeposit(userName, amount, paymentLogObject);
    	}
    	return j;
    }
	...
	public ReceiveRedEnvelope findByReceiveRedEnvelopeId(String receiveRedEnvelopeId){
		...
		if(tableNumber == 0){//默认对象为HistoryOrder
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.redEnvelope.ReceiveRedEnvelope_"+tableNumber);
                //获取无参数构造函数
