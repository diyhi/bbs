package cms.repository.help.impl;
...
@Repository
@Transactional
public class HelpRepositoryImpl extends DaoSupport<Help> implements HelpRepository {
	...
	public Integer saveHelp(Help help){
		...
		//增加帮助数量
		int i = helpTypeRepository.addHelpQuantity(help.getHelpTypeId());
		this.save(help);
		return i;
	}
	/**
	...
	public Integer updateHelp(Help help){
		Query query = em.createQuery("update Help o set o.name=?1, o.content=?2,o.isMarkdown=?3,o.markdownContent=?4,o.helpTypeId=?5 where o.id=?6")
		.setParameter(1, help.getName())
		.setParameter(2, help.getContent())
		.setParameter(3, help.getIsMarkdown())
		.setParameter(4, help.getMarkdownContent())
		.setParameter(5, help.getHelpTypeId())
		.setParameter(6, help.getId());
	...
	public Integer updateHelp(List<Help> helpList,Long new_helpTypeId){
		...
		if(helpList != null && helpList.size() >0){
			for(Help help : helpList){
				...
				Query query = em.createQuery("update Help o set o.helpTypeId=?1, o.content=?2,o.markdownContent=?3 where o.id=?4")
				.setParameter(1, help.getHelpTypeId())
				.setParameter(2, help.getContent())
				.setParameter(3, help.getMarkdownContent())
				.setParameter(4, help.getId());
				i += query.executeUpdate();
				
				//减少旧帮助数量
				helpTypeRepository.minusHelpQuantity(old_helpTypeId,1L);
				//增加新帮助数量
				helpTypeRepository.addHelpQuantity(new_helpTypeId);
			}
		}
		return i;
	}
	...
	public Integer reductionHelp(List<Help> helpList){
		...
		if(helpList != null && helpList.size() >0){
			for(Help help : helpList){
				...
				i += j;
				if(j >0){
					helpTypeRepository.mark_addHelpQuantity(help.getHelpTypeId());
				}
	...
	public Integer markDelete(Long helpTypeId,Long helpId){
		...
		i= query.executeUpdate();
		if(i >0){
			helpTypeRepository.mark_minusHelpQuantity(helpTypeId);
		}
		return i;
	...
	public Integer deleteHelp(Long helpTypeId,Long helpId,Long helpQuantity){
		...
		if(i >0){
			helpTypeRepository.minusHelpQuantity(helpTypeId,helpQuantity);
		}
		return i;
	}
	/**
