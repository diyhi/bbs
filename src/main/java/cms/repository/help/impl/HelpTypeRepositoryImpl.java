...
@Repository
@Transactional
public class HelpTypeRepositoryImpl extends DaoSupport<HelpType> implements HelpTypeRepository {
    @Lazy
    @Resource
	...
	public void saveType(HelpType helpType){
		...
		if(!helpType.getParentId().equals(0L)){//如果不是根节点
			...
			if(j >0){
				...
				//将父节点下的分类帮助转到本节点
                helpRepository.updateHelpTypeId(helpType.getParentId(), helpType.getId());
