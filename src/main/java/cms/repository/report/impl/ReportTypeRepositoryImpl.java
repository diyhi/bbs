...
@Repository
@Transactional
public class ReportTypeRepositoryImpl extends DaoSupport<ReportType> implements ReportTypeRepository {
	...
	*/
	...
	public void saveReportType(ReportType reportType){
		...
		if(!"0".equals(reportType.getParentId())){//如果不是根节点
			...
			if(j >0){
				//将父节点下的举报转到本节点
				reportRepository.updateTypeId(reportType.getParentId(), reportType.getId());
			}
		}
