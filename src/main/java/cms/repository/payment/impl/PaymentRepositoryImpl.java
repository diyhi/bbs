package cms.repository.payment.impl;
...
import cms.component.payment.PaymentLogConfig;
...
@Repository
@Transactional
public class PaymentRepositoryImpl extends DaoSupport<OnlinePaymentInterface> implements PaymentRepository {
	 ...
	 */
	@Transactional(readOnly=true,propagation=Propagation.NOT_SUPPORTED)
	@Cacheable(value="paymentRepositoryImpl_cache",key="'findAllEffectiveOnlinePaymentInterface_default'")
	 ...
	 */
	@CacheEvict(value="paymentRepositoryImpl_cache",allEntries=true)
	...
	public PaymentLog findPaymentLogByPaymentRunningNumber(String paymentRunningNumber){
		...
		if(tableNumber == 0){//默认对象为HistoryOrder
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.payment.PaymentLog_"+tableNumber);
                //获取无参数构造函数
