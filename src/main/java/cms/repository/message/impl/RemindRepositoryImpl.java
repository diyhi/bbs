...
@Repository
@Transactional
public class RemindRepositoryImpl extends DaoSupport<Remind> implements RemindRepository {
	private static final Logger logger = LogManager.getLogger(RemindRepositoryImpl.class);
	...
	public QueryResult<Remind> findRemindByUserId(Long userId,Integer status,int firstIndex, int maxResult){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.message.Remind_"+tableNumber);
                //获取无参数构造函数
