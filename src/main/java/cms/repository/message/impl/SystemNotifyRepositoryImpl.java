package cms.repository.message.impl;
...
import cms.component.message.SubscriptionSystemNotifyConfig;
...
@Repository
@Transactional
public class SystemNotifyRepositoryImpl extends DaoSupport<SystemNotify> implements SystemNotifyRepository {
	private static final Logger logger = LogManager.getLogger(SystemNotifyRepositoryImpl.class);
	...
	public QueryResult<SubscriptionSystemNotify> findSubscriptionSystemNotifyByUserId(Long userId, Integer status, int firstIndex, int maxResult){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.message.SubscriptionSystemNotify_"+tableNumber);
                //获取无参数构造函数
	 ...
	 */
	@Transactional(readOnly=true, propagation=Propagation.NOT_SUPPORTED)
	public Map<Long, LocalDateTime> findSystemNotifyBySendTime(LocalDateTime sendTime){
		Query query = em.createQuery("select o.id,o.sendTime from SystemNotify o where o.sendTime>=?1")
			.setParameter(1, sendTime);

		Map<Long, LocalDateTime> map = new HashMap<Long, LocalDateTime>();
		List<Object[]> objectList = query.getResultList();
		...
		if(objectList != null && objectList.size() >0){
			for(int o = 0; o<objectList.size(); o++){
				...
				map.put(ObjectConversion.conversion(object[0], ObjectConversion.LONG), ObjectConversion.conversion(object[1], ObjectConversion.LOCALDATETIME));
	 ...
	 */
	@Transactional(readOnly=true, propagation=Propagation.NOT_SUPPORTED)
	public Map<Long, LocalDateTime> findSystemNotifyById(Long systemNotifyId){
	 ...
	 */
	@Transactional(readOnly=true, propagation=Propagation.NOT_SUPPORTED)
	public Long findSystemNotifyCountBySendTime(LocalDateTime start_sendTime){
