...
import cms.component.message.PrivateMessageConfig;
...
@Repository
@Transactional
public class PrivateMessageRepositoryImpl extends DaoSupport<PrivateMessage> implements PrivateMessageRepository {
	private static final Logger logger = LogManager.getLogger(PrivateMessageRepositoryImpl.class);
	...
	public QueryResult<PrivateMessage> findPrivateMessageChatByUserId(Long userId,Long friendUserId,Integer status,int firstIndex, int maxResult,Integer sort){
		...
		if(tableNumber == 0){//默认对象
			...
			try {
				//带下划线对象
				Class<?> c = Class.forName("cms.model.message.PrivateMessage_"+tableNumber);
                //获取无参数构造函数
