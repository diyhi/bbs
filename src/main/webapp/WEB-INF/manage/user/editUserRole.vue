<!-- 修改角色 -->
<template id="editUserRole-template">
	<div>
		<div class="main">
			<div class="navbar">
...
//修改角色
export default({
	name: 'editUserRole',//组件名称，keep-alive缓存需要本参数
	template : '#editUserRole-template',
	inject:['reload'], 
	data : function data() {
		return {
			userRoleId :'',
	...
	created : function created() {
		...
		if(this.$route.query.userRoleId != undefined && this.$route.query.userRoleId != ''){
			this.userRoleId = this.$route.query.userRoleId;	
		}
		
		this.queryEditUserRole();
	},
	methods : {
		//处理全选单元格
		...
		//处理用户资源全选
		handleUserResourceSelectAll: function(){
			if(this.userResourceGroupList != null && this.userResourceGroupList.length >0){
				...
				for (let i = 0; i <this.userResourceGroupList.length; i++) {
					...
					if(userResourceGroup.userResourceList != null && userResourceGroup.userResourceList.length >0){
						...
						if(count == userResourceGroup.userResourceList.length){
							this.$refs.table.toggleRowSelection(this.userResourceGroupList[userResourceGroup.index],true);
						}
					}
				}
			}
			    	...
			    	if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "userRole"){
			    				...
			    				_self.name = userRole.name;
								_self.sort = userRole.sort;
					    		...
					    		_self.$nextTick(function() {
					    			_self.handleUserResourceSelectAll();
					    		});
			    			}
			    		}
