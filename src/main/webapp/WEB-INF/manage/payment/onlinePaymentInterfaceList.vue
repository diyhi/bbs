<!-- 在线支付接口列表 -->
<template id="onlinePaymentInterfaceList-template">
	<div>
		<div class="main">
			<div class="navbar">
				<el-button type="primary" plain size="small" @click="$router.push({path: '/admin/control/onlinePaymentInterface/manage/add'});">添加在线支付接口</el-button>
			</div>
					...
					<el-table-column label="操作" align="center" width="200">
						<template #default="scope">
							<div class="button-group-wrapper">
								<el-button-group>
									<el-button type="primary" size="mini" @click="$router.push({path: '/admin/control/onlinePaymentInterface/manage/edit', query:{ onlinePaymentInterfaceId : scope.row.id, page:currentpage}})">修改</el-button>
									<el-button type="primary" size="mini" @click="deleteOnlinePaymentInterface($event,scope.row)">删除</el-button>
								</el-button-group>
							</div>
				    	</template>
...
//在线支付接口列表
export default({
	name: 'onlinePaymentInterfaceList',//组件名称，keep-alive缓存需要本参数
	template : '#onlinePaymentInterfaceList-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	created : function created() {
		...
		//初始化
		this.queryOnlinePaymentInterfaceList();
	},
	methods : {
		//查询在线支付接口列表
		queryOnlinePaymentInterfaceList : function() {
			let _self = this;
			...
			_self.$ajax.get('control/onlinePaymentInterface/list', {
			    params: {
		...
		//删除在线支付接口
	    deleteOnlinePaymentInterface : function(event,row) {
			//强制失去焦点
				...
				.then(function (response) {
				    ...
				    if(result){
				    	...
				    	if(returnValue.code === 200){//成功
				    		...
				    		_self.queryOnlinePaymentInterfaceList();
				    	}else if(returnValue.code === 500){//错误
