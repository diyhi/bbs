...
<template id="topicList-template">
	<div>
		<div class="main">
			<div class="navbar">
				<el-button type="primary" plain size="small" v-if="visible == 'false'" @click="$router.push({path: '/admin/control/topic/list', query:{ visible : true}})">返回</el-button>
				<el-button type="primary" plain size="small" v-if="visible == 'false'" @click="reductionTopic($event)">还原</el-button>
				<el-button type="primary" plain size="small" v-if="visible == 'true'" @click="$router.push({path: '/admin/control/topic/manage/add', query:{ visible : true,page:($route.query.page != undefined ? $route.query.page:'')}})">添加话题</el-button>
				<el-button type="primary" plain size="small" v-if="visible == 'true'" @click="$router.push({path: '/admin/control/topic/list', query:{ visible : false}})">回收站</el-button>
					...
					<el-table-column label="操作" align="center" min-width="120">
						<template #default="scope">
							<div class="button-group-wrapper">
								<el-button-group>
									<el-button type="primary" size="mini" @click="$store.commit('setCacheNumber');$router.push({path: '/admin/control/topic/manage/view', query:{ visible : ($route.query.visible != undefined ? $route.query.visible:''),topicId : scope.row.id}})">查看</el-button>
