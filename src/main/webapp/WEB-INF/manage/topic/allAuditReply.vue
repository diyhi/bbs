<!-- 全部待审核回复 -->
<template id="allAuditReply-template">
	<div>
		<div class="main">
			<div class="navbar">
					...
					<el-table-column label="操作" align="center" width="150">
						<template #default="scope">
							<div class="button-group-wrapper">
								<el-button-group>
									<el-button type="primary" size="mini" @click="$store.commit('setCacheNumber');$router.push({path: '/admin/control/topic/manage/view', query:{ topicId : scope.row.topicId, commentId:scope.row.commentId, replyId:scope.row.id}})">查看</el-button>
...
//全部待审核回复
export default({
	name: 'allAuditReply',//组件名称，keep-alive缓存需要本参数
	template : '#allAuditReply-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	methods : {
		...
		queryReplyList : function() {
			...
			_self.tableData = []; 
			 ...
			 _self.$ajax.get('control/topic/allAuditReply', {
			    params: {
