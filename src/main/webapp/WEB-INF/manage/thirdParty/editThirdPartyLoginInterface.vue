<!-- 修改第三方登录接口 -->
<template id="editThirdPartyLoginInterface-template">
	<div>
		<div class="main" >
			<div class="navbar">
			...
			<div class="data-form label-width-blank" >
				<el-form label-width="auto"  @submit.native.prevent>
					...
					<el-form-item label="接口产品" :required="true" :error="error.interfaceProduct">
						<span v-if="interfaceProduct == 10">微信</span>
					</el-form-item>
					...
					<el-form-item label="注意" v-if="interfaceProduct ==10">
						<div class="form-help" >
			    			...
			    			2、需要在微信开放平台的“管理中心 - 开发信息 - 授权回调域”的配置选项中，修改授权回调域名 (域名格式不需要加http://协议头)<br>
							3、需要在微信开放平台绑定公众号
						</div>
					</el-form-item>
...
//修改第三方登录接口
export default({
	name: 'editThirdPartyLoginInterface',//组件名称，keep-alive缓存需要本参数
	template : '#editThirdPartyLoginInterface-template',
	inject:['reload'], 
	data : function data() {
		return {
	...
	created : function created() {
		...
		if(this.$route.query.thirdPartyLoginInterfaceId != undefined && this.$route.query.thirdPartyLoginInterfaceId != ''){
			this.thirdPartyLoginInterfaceId = this.$route.query.thirdPartyLoginInterfaceId;
		}
		
		
		this.queryEditThirdPartyLoginInterface();
	},
	methods : {
		//查询修改第三方登录接口
	    queryEditThirdPartyLoginInterface: function(){
	        let _self = this;
			...
			_self.$ajax.get('control/thirdPartyLoginInterface/manage', {
			    params: {
			    	method : 'edit',
			    	thirdPartyLoginInterfaceId :_self.thirdPartyLoginInterfaceId
			    }
			})
			    	...
			    	if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "thirdPartyLoginInterface"){
			    				...
			    				_self.name = thirdPartyLoginInterface.name;
			    				_self.interfaceProduct = thirdPartyLoginInterface.interfaceProduct;
			    				_self.enable = thirdPartyLoginInterface.enable;
			    				_self.sort = thirdPartyLoginInterface.sort;
			    			}else if(key == "weChatConfig"){
			    				let weChatConfig = mapData[key];
			    				_self.weixin_op_appID = weChatConfig.op_appID;
			    				_self.weixin_op_appSecret = weChatConfig.op_appSecret;
			    				_self.weixin_oa_appID = weChatConfig.oa_appID;
			    				_self.weixin_oa_appSecret = weChatConfig.oa_appSecret;
			    			}
			    		}
		...
		submitForm : function() {
			...
			formData.append('thirdPartyLoginInterfaceId', _self.thirdPartyLoginInterfaceId);
			...
			_self.$ajax({
		        method: 'post',
		        url: 'control/thirdPartyLoginInterface/manage?method=edit',
		        data: formData
			})
			.then(function (response) {
				if(response == null){
