<!-- 话题举报列表 -->
<template id="topicReportList-template">
	<div>
		<div class="main topicReportModule">
			<div class="navbar">
	            <el-button type="primary" size="small" plain @click="$router.push({path: '/admin/control/topic/manage/view', query:{ visible:($route.query.visible != undefined ? $route.query.visible:''),topicView_beforeUrl:($route.query.topicView_beforeUrl != undefined ? $route.query.topicView_beforeUrl:''),topicId :$route.query.topicId, commentId:($route.query.commentId != undefined ? $route.query.commentId:''), page:($route.query.topicPage != undefined ? $route.query.topicPage:'')}})">返回</el-button>
...
//话题举报列表
export default({
	name: 'topicReportList',//组件名称，keep-alive缓存需要本参数
	template : '#topicReportList-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	created : function created() {
		...
		//初始化
		this.queryTopicReportList();
	},
	methods : {

		//查询话题举报列表
		queryTopicReportList : function() {
			let _self = this;
			
			//清空内容
			_self.tableData = []; 
			
			_self.$ajax.get('control/topicReport/list', {
			    params: {
