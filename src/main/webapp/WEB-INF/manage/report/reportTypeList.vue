<!-- 举报分类列表 -->
<template id="reportTypeList-template">
	<div>
		<div class="main">
			...
			<div class="navbar">
				<el-button type="primary" plain size="small" @click="$router.push({path: '/admin/control/reportType/manage/add',query:{sourceParentId:($route.query.parentId != undefined ? $route.query.parentId:''), parentId : parentId,page:($route.query.page != undefined ? $route.query.page:'')}});">添加分类</el-button>
			</div>
			<div class="data-table" >
				<el-table :data="tableData" tooltip-effect="dark" style="width: 100%" @cell-click="cellExpandRow"  stripe empty-text="没有内容">
					...
					<el-table-column label="操作" align="center" width="300">
						<template #default="scope">
							<el-button-group>
								<el-button type="primary" size="mini" @click="$router.push({path: '/admin/control/reportType/manage/add',query:{sourceParentId:($route.query.parentId != undefined ? $route.query.parentId:''), parentId: scope.row.id,page:($route.query.page != undefined ? $route.query.page:'')}});">添加子分类</el-button>
								<el-button type="primary" size="mini" @click="$router.push({path: '/admin/control/reportType/manage/edit',query:{ sourceParentId:($route.query.parentId != undefined ? $route.query.parentId:''),typeId: scope.row.id,parentId: scope.row.parentId,page:($route.query.page != undefined ? $route.query.page:'')}});">修改</el-button>
...
//举报分类列表
export default({
	name: 'reportTypeList',//组件名称，keep-alive缓存需要本参数
	template : '#reportTypeList-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	methods : {
		...
		queryTypeList : function(parentId,page) {
			...
			_self.$ajax.get('control/reportType/list', {
			    params: {
			    	parentId : parentId,
			    	page : page
			    }
