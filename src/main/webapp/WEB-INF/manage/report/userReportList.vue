<!-- 用户举报列表 -->
<template id="userReportList-template">
	<div>
		<div class="main">
	        <div class="navbar">
	            <el-button type="primary" size="small" plain @click="$router.push({path: '/admin/control/user/manage/show', query:{ id : $route.query.id,beforeUrl:($route.query.beforeUrl != undefined ? $route.query.beforeUrl:'')}})">返回</el-button>
...
//用户举报列表
export default({
	name: 'userReportList',//组件名称，keep-alive缓存需要本参数
	template : '#userReportList-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	methods : {
		...
		//查询用户举报列表
		queryUserReportList : function() {
			let _self = this;
			
			//清空内容
			_self.tableData = []; 
			
			_self.$ajax.get('control/userReport/list', {
			    params: {
