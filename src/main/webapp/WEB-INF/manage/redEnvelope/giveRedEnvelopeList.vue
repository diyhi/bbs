<!-- 发红包列表 -->
<template id="giveRedEnvelopeList-template">
	<div>
		<div class="main">
			<div class="navbar">
			...
			<div class="data-table" >
				<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
					...
					<el-table-column label="话题" align="center" >
						<template #default="scope">
							<el-link href="javascript:void(0);" @click="$store.commit('setCacheNumber'); $router.push({path: '/admin/control/topic/manage/view', query:{ topicId : scope.row.bindTopicId,commentId:''}})">{{scope.row.bindTopicTitle}}</el-link>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" align="center" width="100">
						<template #default="scope">
							<el-button-group>
								<el-button type="primary" size="mini" @click="$router.push({path: '/admin/control/redEnvelope/redEnvelopeAmountDistribution/list', query:{ giveRedEnvelopeId : scope.row.id, giveRedEnvelopePage:currentpage, id : $route.query.id,userName : encodeURIComponent(userName),beforeUrl:($route.query.beforeUrl != undefined ? $route.query.beforeUrl:'')}})">金额分配</el-button>
							</el-button-group>
				    	</template>
					</el-table-column>
				</el-table>
	...
	methods : {
		...
		queryGiveRedEnvelopeList : function() {
			...
			_self.tableData = []; 
			 ...
			 _self.$ajax.get('control/redEnvelope/giveRedEnvelope/list', {
				 params: {
					id :_self.id,
