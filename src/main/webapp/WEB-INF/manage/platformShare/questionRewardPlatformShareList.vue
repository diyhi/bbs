<!-- 问答悬赏平台分成列表 -->
<template id="questionRewardPlatformShareList-template">
	<div>
		<div class="main">
			<div class="search">
				...
				<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" stripe empty-text="没有内容">
					<el-table-column label="问题标题" align="center" >
						<template #default="scope">
							<el-link href="javascript:void(0);" @click="$store.commit('setCacheNumber'); $router.push({path: '/admin/control/question/manage/view', query:{ questionId : scope.row.questionId,answerId:''}})">{{scope.row.questionTitle}}</el-link>
						</template>
					</el-table-column>
...
//问答悬赏平台分成列表
export default({
	name: 'questionRewardPlatformShareList',//组件名称，keep-alive缓存需要本参数
	template : '#questionRewardPlatformShareList-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	created : function created() {
		...
		//初始化
		this.queryQuestionRewardPlatformShareList();
	},
	methods : {
		//查询问答悬赏平台分成列表
		queryQuestionRewardPlatformShareList : function() {
			let _self = this;
			
			//清空内容
			_self.tableData = []; 
			
			_self.$ajax.get('control/questionRewardPlatformShare/list', {
			    params: {
