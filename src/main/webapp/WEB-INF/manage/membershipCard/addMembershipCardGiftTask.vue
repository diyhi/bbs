<!-- 添加会员卡赠送任务 -->
<template id="addMembershipCardGiftTask-template">
	<div>
		<div class="main">
			<div class="navbar">
...
//添加会员卡赠送任务
export default({
	name: 'addMembershipCardGiftTask',//组件名称，keep-alive缓存需要本参数
	template : '#addMembershipCardGiftTask-template',
	inject:['reload'], 
	data : function data() {
		return {
			rolesList:'',
	...
	created : function created() {
		...
		this.queryAddMembershipCardGiftTask();
	},
	methods : {
	
	
		//查询添加会员卡赠送任务
	    queryAddMembershipCardGiftTask: function(){
	        let _self = this;
		...
		submitForm : function() {
			...
			.then(function (response) {
			    ...
			    if(result){
			    	...
			    	if(returnValue.code === 200){//成功
			    		if(_self.type == 20){
			    			_self.$message.success("提交成功，任务开始异步运行");
			    		}else{
