<!-- 修改角色 -->
<template id="editRoles-template">
	<div>
		<div class="main">
			<div class="navbar">
...
//修改角色
export default({
	name: 'editRoles',//组件名称，keep-alive缓存需要本参数
	template : '#editRoles-template',
	inject:['reload'], 
	data : function data() {
		return {
			rolesId:'',
	...
	created : function created() {
		...
		if(this.$route.query.rolesId != undefined && this.$route.query.rolesId != ''){
			this.rolesId = this.$route.query.rolesId;
		}
	...
	methods : {
			    	...
			    	if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "roles"){
			    				...
			    				_self.name= roles.name;
			    				_self.remarks = roles.remarks;
		...
		submitForm : function() {
			...
			formData.append('rolesId', _self.rolesId);
			if(_self.name != null){
