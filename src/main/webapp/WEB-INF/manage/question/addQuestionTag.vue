<!-- 添加问题标签 -->
<template id="addQuestionTag-template">
	<div>
		<div class="main">
			<div class="nav-breadcrumb">
				<el-breadcrumb separator-class="el-icon-arrow-right">
					<el-breadcrumb-item @click="$router.push({path: '/admin/control/questionTag/list'});">全部标签</el-breadcrumb-item>
					<el-breadcrumb-item v-for="(value, key) in navigation" @click="$router.push({path: '/admin/control/questionTag/list',query:{parentId:key}});" @click="queryQuestionTag(1,key)">{{value}}</el-breadcrumb-item>
				</el-breadcrumb>
			</div>
			<div class="navbar">
...
//添加帮助分类
export default({
	name: 'addQuestionTag',//组件名称，keep-alive缓存需要本参数
	template : '#addQuestionTag-template',
	inject:['reload'], 
	data : function data() {
		return {
			parentId:'',
