<!-- 全部待审核答案回复 -->
<template id="allAuditAnswerReply-template">
	<div>
		<div class="main">
			<div class="navbar">
				<el-button type="primary" plain size="small"  @click="deleteReply($event)">批量删除</el-button>
			</div>
			<div class="data-table" >
				<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" stripe empty-text="没有内容">
					...
					<el-table-column prop="questionTitle" label="标题" width="180">
					...
					<el-table-column prop="postTime" label="回复时间" align="center" width="150"></el-table-column>
					<el-table-column label="操作" align="center" width="150">
						<template #default="scope">
							<div class="button-group-wrapper">
								<el-button-group>
									<el-button type="primary" size="mini" @click="$store.commit('setCacheNumber');$router.push({path: '/admin/control/question/manage/view', query:{ questionId : scope.row.questionId, answerId:scope.row.answerId, replyId:scope.row.id}})">查看</el-button>
									<el-button type="primary" size="mini" @click="deleteReply($event,scope.row)">删除</el-button>
								</el-button-group>
							</div>
				    	</template>
...
//全部待审核答案回复
export default({
	name: 'allAuditAnswerReply',//组件名称，keep-alive缓存需要本参数
	template : '#allAuditAnswerReply-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	methods : {
		...
		queryReplyList : function() {
			...
			_self.tableData = []; 
			 ...
			 _self.$ajax.get('control/question/allAuditAnswerReply', {
			    params: {
	    ...
	    //删除回复
	    deleteReply : function(event,row) {
	    	//强制失去焦点
