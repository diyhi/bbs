<!-- 全部待审核答案 -->
<template id="allAuditAnswer-template">
	<div>
		<div class="main">
			<div class="navbar">
				<el-button type="primary" plain size="small"  @click="deleteAnswer($event)">批量删除</el-button>
			</div>
			<div class="data-table" >
				<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" stripe empty-text="没有内容">
					...
					<el-table-column prop="postTime" label="回答时间" align="center" width="150"></el-table-column>
					<el-table-column label="操作" align="center" width="150">
						<template #default="scope">
							<div class="button-group-wrapper">
								<el-button-group>
									<el-button type="primary" size="mini" @click="$store.commit('setCacheNumber');$router.push({path: '/admin/control/question/manage/view', query:{ questionId : scope.row.questionId,answerId:scope.row.id}})">查看</el-button>
									<el-button type="primary" size="mini" @click="deleteAnswer($event,scope.row)">删除</el-button>
								</el-button-group>
							</div>
				    	</template>
...
//全部待审核答案
export default({
	name: 'allAuditAnswer',//组件名称，keep-alive缓存需要本参数
	template : '#allAuditAnswer-template',
	inject:['reload'], 
	data : function data() {
		return {
			tableData: [],//表格内容
	...
	methods : {
		...
		queryAnswerList : function() {
			...
			_self.$ajax.get('control/question/allAuditAnswer', {
			    params: {
	    ...
	    //删除答案
	    deleteAnswer : function(event,row) {
	    	//强制失去焦点
